...
let nextUserId =
  users.reduce((maximum, user) => Math.max(maximum, Number(user.id) || 0), 0) + 1;

let nextTransactionId =
  transactions.reduce(
    (maximum, transaction) => Math.max(maximum, Number(transaction.id) || 0),
    0
  ) + 1;
...
app.post("/api/register", (req, res) => {
  ...
  if (password.length < 6 || password.length > 128) {
    return res.status(400).json({ error: "Salasanan pitää olla 6–128 merkkiä" });
  }
  ...
  try {
    ...
    users.pop();
    nextUserId--;
    return res.status(500).json({ error: "Tilin tallentaminen epäonnistui" });
  }
...
app.post("/api/login", (req, res) => {
  ...
  req.session.regenerate((error) => {
    if (error) {
      ...
      return res.status(500).json({ error: "Istunnon luominen epäonnistui" });
    }
    ...
    req.session.save((saveError) => {
      if (saveError) {
        ...
        return res.status(500).json({ error: "Istunnon tallentaminen epäonnistui" });
      }
...
app.get("/api/me", requireAuth, (req, res) => {
  ...
  if (!user) {
    return req.session.destroy(() => {
      res.status(401).json({ error: "Kirjaudu sisään" });
    });
  }
...
app.get("/api/user/:id", requireAuth, (req, res) => {
  ...
  if (!Number.isSafeInteger(id) || id < 1) {
    return res.status(400).json({ error: "Virheellinen käyttäjä-ID" });
  }
...
app.get("/api/transactions", requireAuth, (req, res) => {
  ...
  const result = transactions
    .filter(
      (transaction) =>
        transaction.from_user === myId || transaction.to_user === myId
    )
    .sort(
      (a, b) =>
    ...
    .slice(0, 30)
    .map((transaction) => {
      const fromUser = users.find((user) => user.id === transaction.from_user);
      const toUser = users.find((user) => user.id === transaction.to_user);
...
app.post("/api/transfer", requireAuth, (req, res) => {
  ...
  if (!Number.isSafeInteger(toId) || toId < 1) {
    return res.status(400).json({ error: "Vastaanottajan ID puuttuu" });
  }
  ...
  if (!sender) return res.status(401).json({ error: "Kirjaudu sisään" });
  ...
  if (sender.balance < cents) {
    return res.status(400).json({ error: "Saldo ei riitä" });
  }
  ...
  try {
    ...
    transactions.pop();
    nextTransactionId--;
    return res.status(500).json({ error: "Siirron tallennus epäonnistui" });
  }
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`Pukku server running on port ${PORT}`);
});
